export const config = {
  ...
  rconHost: process.env.MC_RCON_HOST ?? "minecraft",
  rconPort: Number(process.env.MC_RCON_PORT ?? 25575),
  ...
  catalogUserAgent: process.env.CATALOG_USER_AGENT ?? `MCServer-Panel/${process.env.APP_VERSION ?? "1.1.2"} (https://github.com/DooSys/MCServer-Panel)`
};

export function assertRuntimeConfig() {
  if (!config.rconPassword) {
    console.warn("[config] MC_RCON_PASSWORD is empty. RCON endpoints will fail until it is set.");
  }
}
