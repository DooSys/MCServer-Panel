...
```bash
npm version 1.1.18 --no-git-tag-version
```
...
- Synology compose should consume a pinned release tag such as `ghcr.io/doosys/mcserver-panel:1.1.18` during validation.
...
- Version 1.1.18 adds /panel-api/logs/sources and /panel-api/logs/:source.
...
- Version 1.1.18 proxies PocketBase admin at /_/ because the PocketBase dashboard calls root /api/collections endpoints.
...
- Version 1.1.18 separates browser auth storage: MCServer-panel uses `mcserver_panel_auth`; PocketBase admin keeps the default `pocketbase_auth`.
...
- Version 1.1.18 reads and writes gamerules by trying the project snake_case key first, then the legacy camelCase alias.
...
- Since 1.1.18, PocketBase owns `/api` and `/_/` at the public panel host.
...
- Version 1.1.18 expands Modrinth catalog search to 100 results per compatible type.
...
- Version 1.1.18 adds Minecraft/Paper update status from detected server flavor and version; set `MINECRAFT_UPDATE_CHECK=false` to disable network checks.
...
- Version 1.1.18 refreshes the visual identity around the supplied MCServer-panel artwork.
